...
const Dashboard = () => {
  ...
  const handleAddNewSpace = async (newCanvasName) => {
    ...
    const data = {
      title: newCanvasName,
      canvas_id: newId, // Use the correct attribute name as per your backend model
    };
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log("Failed to create new canvas:", response.status);
      }
    } catch (error) {
  ...
  useEffect(() => {
    console.log("Space List:", spacesList);
    console.log(
      "First Space ID:",
      spacesList.length > 0 ? spacesList[0].id : "No spaces available"
    );
  }, [spacesList]);
    ...
    const fetchDataAndNavigate = async () => {
      ...
      try {
        const response = await fetch(
          `http://127.0.0.1:8000/canvas/get/${id}/`,
          {
            method: "GET",
    ...
    >
      ...
      <Stack direction={"row"} spacing={2} mt={1}>
        ...
        >
          ...
          {spacesList.map((space) => {
            console.log(spacesList[0].id);
            console.log("Space ID:", space.id); // Correct placement of console.log
            //
            return (
              <SpaceItemCard
                key={space.id}
...
const SpaceItemCardMemo = React.memo(SpaceItemCard);
const OnlineUserCardMemo = React.memo(OnlineUserCard);
